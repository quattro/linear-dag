...
from .brick_graph import merge_brick_graphs
from .one_summed_cy import linearize_brick_graph
from .lineararg import VariantInfo, LinearARG
...
def make_genotype_matrix(vcf_path, linarg_dir, region, partition_number, phased=True, flip_minor_alleles=False):
    ...
    for var in vcf(region_formatted):
        ...
        f_var.write(' '.join([chrom, str(var.POS), '.', var.REF, ','.join(var.ALT), str(int(flip)), str(var_index)])+'\n')
        var_index += 1
...
def infer_brick_graph(linarg_dir, load_dir, partition_identifier):
    ...
    """
    if not os.path.exists(f'{linarg_dir}/brick_graph_partitions/'): os.makedirs(f'{linarg_dir}/brick_graph_partitions/')
    if not os.path.exists(f'{linarg_dir}/brick_graph_partition_stats/'): os.makedirs(f'{linarg_dir}/brick_graph_partition_stats/')
    ...
    np.savez(f'{linarg_dir}/brick_graph_partitions/{partition_identifier}.npz',
    ...
    stats = [partition_identifier, str(n), str(m), str(np.round(t2-t1, 3)), str(np.round(t3-t2, 3)), str(np.round(t4-t3, 3)), str(geno_nnz), str(adj_mat.nnz), str(np.round(nnz_ratio, 3))]

    with open(f'{linarg_dir}/brick_graph_partition_stats/{partition_identifier}.txt', 'w') as file:
        file.write(" ".join(['partition_identifier', 'n', 'm', 'mtx_load_time', 'brickgraph_inference_time', 'find_recombinations_time', 'geno_nnz', 'brickgraph_nnz', 'nnz_ratio'])+'\n')
...
def merge(linarg_dir, load_dir):
    ...
    """
    if not os.path.exists(f'{linarg_dir}/'): os.makedirs(f'{linarg_dir}/')
    merged_graph, variant_indices, num_samples, index_mapping = merge_brick_graphs(f'{load_dir}{linarg_dir}/brick_graph_partitions')
...
def get_linarg_stats(linarg_dir, load_dir):
    ...
    stats = [linarg.shape[0], linarg.shape[1], genotypes_nnz, linarg.nnz, brick_graph_nnz, np.round(genotypes_nnz/linarg.nnz, 3), np.round(genotypes_nnz/brick_graph_nnz, 3), all(allele_counts_from_genotype == allele_count_from_linarg[0])]
    stats = [str(x) for x in stats]
    with open(f'{linarg_dir}/linear_arg_stats.txt', 'w') as file:
        file.write(" ".join(['n', 'm', 'genotypes_nnz', 'linarg_nnz', 'brick_graph_nnz', 'linarg_nnz_ratio', 'brick_graph_nnz_ratio', 'correct_allele_counts'])+'\n')
        file.write(" ".join(stats)+'\n')