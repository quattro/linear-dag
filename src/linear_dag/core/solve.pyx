...
def spsolve_backward_triangular_matmat_one_sparse(
    ...
    if b.ndim == 1:
        b = b.reshape(1, -1)
    if stop_at_node is not None:
        _spsolve_backward_triangular_matmat_one_sparse_node_limit(
            A, b, nonunique_indices, min_index_to_keep, stop_at_node
        )
        return
    if b.dtype == np.float64:
...
def _spsolve_backward_triangular_matmat_one_sparse_node_limit(
    A,
    b: np.ndarray,
    nonunique_indices: np.ndarray,
    min_index_to_keep: int,
    ...
    """Solve over the leading node subgraph for compressed edge weights."""
    if node_limit < 0 or node_limit > A.shape[0]:
        raise ValueError(f"node_limit must be between 0 and {A.shape[0]}, got {node_limit}.")
    edge_limit = int(A.indptr[node_limit])
    nonunit_idx = int(np.searchsorted(A.nonunit_edge_indices, edge_limit, side="left")) - 1
    if b.dtype == np.float64:
        _spsolve_backward_triangular_matmat_one_sparse_node_limit_float64(
            A.indptr,
            A.indices,
            A.nonunit_edge_indices,
            A.nonunit_values,
            b,
            nonunique_indices,
            min_index_to_keep,
        ...
        _spsolve_backward_triangular_matmat_one_sparse_node_limit_float32(
            A.indptr,
            A.indices,
            A.nonunit_edge_indices,
            A.nonunit_values,
...
cdef void _spsolve_backward_triangular_matmat_one_sparse_node_limit_float64(
    int[:] indptr, int[:] indices, int[:] nonunit_indices, int[:] nonunit_values,
    double[:, :] b_view, int[:] nonunique_indices, int min_index_to_keep,
    int node_limit, int nonunit_idx
) noexcept nogil:
    cdef int node_idx, edge_idx = indptr[node_limit]
    cdef int neighbor_idx
    cdef int vector_length = b_view.shape[0]
    cdef int vector_bytes = vector_length * sizeof(double)
    cdef int inc = 1
    ...
    for node_idx in range(node_limit - 1, -1, -1):
        ...
        while edge_idx > indptr[node_idx]:
            edge_idx -= 1
            if nonunit_idx >= 0 and nonunit_indices[nonunit_idx] == edge_idx:
                alpha = <double> nonunit_values[nonunit_idx]
                nonunit_idx -= 1
            else:
                alpha = 1.0
            ...
            if neighbor_idx >= node_limit:
                continue
            source_ptr = &b_view[0, nonunique_indices[neighbor_idx]]
            blas.daxpy(&vector_length, &alpha, source_ptr, &inc, destination_ptr, &inc)


cdef void _spsolve_backward_triangular_matmat_one_sparse_node_limit_float32(
    int[:] indptr, int[:] indices, int[:] nonunit_indices, int[:] nonunit_values,
    float[:, :] b_view, int[:] nonunique_indices, int min_index_to_keep,
    int node_limit, int nonunit_idx
) noexcept nogil:
