...
class LinearARG(LinearOperator):
    ...
    def from_genotypes(
        ...
        linarg = LinearARG(
            A_tri, variants_idx_tri, flip, len(samples_idx), None, VariantInfo(variant_info), iids=pl.Series(iids)
        )
            ...
            if self.variants is not None:
                for field in self.variants.req_fields:
                    if field == "POS":
                        destination.create_dataset(
                            field,
                            data=np.array(self.variants.table[field]).astype(int),
                        ...
                        destination.create_dataset(
                            field,
                            data=np.array(self.variants.table[field]).astype("S"),
                            compression=compression_option,
                            shuffle=True,
                        )
        return
