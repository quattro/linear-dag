...
def from_hdf5_block(
    ...
    **Returns:**
    ...
    with h5py.File(_hdf5_path(path), "r") as file:
        arrays = _read_block_arrays_from_group(file[block], dtype=dtype, cast_data=False)
    return from_block_arrays(arrays, backend=backend, dtype=dtype)
...
def read_hdf5_blocks(
    ...
    **Returns:**
    ...
    with h5py.File(_hdf5_path(path), "r") as file:
        return tuple(
            from_block_arrays(
                _read_block_arrays_from_group(file[block_name], dtype=dtype, cast_data=False),
                backend=backend,
                dtype=dtype,
            )
...
def _read_block_arrays_from_group(
    group: Any,
    *,
    dtype: Any = None,
    cast_data: bool = True,
    ...
    return LinearARGBlockArrays(
        ...
        data=np.asarray(group["data"][:], dtype=np.dtype(dtype)) if cast_data else np.asarray(group["data"][:]),
