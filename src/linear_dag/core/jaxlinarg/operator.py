...
_EDGE_VALIDATION_CHUNK_SIZE = 262_144
_FFI_CPU_DTYPES = {np.dtype(np.float32), np.dtype(np.float64)}
...
def _resolve_backend_for_dtype(requested: Backend, dtype: Any) -> Backend:
    requested = Backend(requested)
    dtype = np.dtype(dtype)
    if requested is Backend.FFI_CPU and dtype not in _FFI_CPU_DTYPES:
        raise ValueError(f"FFI CPU supports only float32 and float64 computation dtypes, got {dtype}")
    if requested is Backend.AUTO and dtype not in _FFI_CPU_DTYPES:
        return Backend.PURE_JAX
...
class JaxLinearARG(eqx.Module):
    ...
    **Arguments:**
    ...
    data: Array = eqx.field(converter=jnp.asarray)
    ...
    def from_lineararg_arrays(
        ...
        **Raises:**
        ...
        backend = _resolve_backend_for_dtype(backend, dtype)

        indptr = np.asarray(indptr, dtype=np.int32)
        indices = np.asarray(indices, dtype=np.int32)
        data = _canonical_edge_data(data, backend=backend, dtype=dtype)
        ...
        return cls(
            ...
            variant_indices=jax.device_put(variant_indices),
            flip=jax.device_put(flip),
            _flipped_variant_indices=jax.device_put(np.asarray(np.flatnonzero(flip), dtype=np.int32)),
            sample_indices=jax.device_put(sample_indices),
            nonunique_indices=jax.device_put(nonunique_indices),
            allele_counts=jax.device_put(allele_counts),
            n_variants=n_variants,
            n_samples=n_samples,
            n_nonunique_indices=n_nonunique_indices,
            min_index_to_keep=min_index_to_keep,
    ...
    def __check_init__(self) -> None:
        resolved_backend = _resolve_backend_for_dtype(self.backend, self.dtype)
        if resolved_backend is not self.backend:
        ...
        if self.backend is Backend.FFI_CPU:
            _validate_ffi_cpu_array_dtypes(arrays, dtype=self.dtype)
    ...
    def _matmat_rank2(self, x: Array) -> Array:
        if self.backend is Backend.FFI_CPU and self.data.dtype == jnp.int32:
            return ffi_cpu.ffi_cpu_matmat(
                self.indptr,
                self.indices,
                self.data,
    ...
    def _rmatmat_rank2(self, x: Array) -> Array:
        if self.backend is Backend.FFI_CPU and self.data.dtype == jnp.int32:
            return ffi_cpu.ffi_cpu_rmatmat(
                self.indptr,
                self.indices,
                self.data,
                self.nonunique_indices,
...
def _canonical_edge_data(data: ArrayLike, *, backend: Backend, dtype: Any) -> np.ndarray:
    values = np.asarray(data)
    if backend is not Backend.FFI_CPU:
        return np.asarray(values, dtype=np.dtype(dtype))
    if not np.issubdtype(values.dtype, np.number):
        raise ValueError(f"FFI CPU requires integer LinearARG edge weights, got {values.dtype}")
    info = np.iinfo(np.int32)
    if np.issubdtype(values.dtype, np.integer):
        if values.size and (np.min(values) < info.min or np.max(values) > info.max):
            raise ValueError("FFI CPU LinearARG edge weights do not fit in int32")
        return np.asarray(values, dtype=np.int32)
    if values.size:
        if not np.all(np.isfinite(values)):
            raise ValueError("FFI CPU requires finite integer LinearARG edge weights")
        minimum = np.min(values)
        maximum = np.max(values)
        if minimum < info.min or maximum > info.max:
            raise ValueError("FFI CPU LinearARG edge weights do not fit in int32")
    exact = np.asarray(values, dtype=np.int32)
    if values.size and not np.array_equal(values, exact):
        raise ValueError("FFI CPU requires integer LinearARG edge weights")
    return exact
...
def _validate_array_contract(
    ...
    *,
    ...
    if _has_invalid_edge_order(indptr, indices, data):
...
def _validate_ffi_cpu_array_dtypes(arrays: dict[str, np.ndarray], *, dtype: Any) -> None:
    if np.dtype(dtype) not in _FFI_CPU_DTYPES:
        raise ValueError(f"FFI CPU supports only float32 and float64 computation dtypes, got {np.dtype(dtype)}")
    ...
    for name in (
        ...
        if arrays[name].dtype != np.dtype(np.int32):
            raise ValueError(f"FFI CPU {name} must have dtype int32, got {arrays[name].dtype}")

    if arrays["flip"].dtype != np.dtype(np.bool_):
        raise ValueError(f"FFI CPU flip must have dtype bool, got {arrays['flip'].dtype}")

    data_dtype = arrays["data"].dtype
    if data_dtype == np.dtype(np.int32):
        return
    if np.issubdtype(data_dtype, np.floating) and data_dtype != np.dtype(dtype):
        raise ValueError(
            f"FFI CPU floating data dtype must match computation dtype {np.dtype(dtype)}, got {data_dtype}"
        )
    if not np.issubdtype(data_dtype, np.floating):
        raise ValueError(f"FFI CPU data must have dtype int32 or {np.dtype(dtype)}, got {data_dtype}")


def _has_invalid_edge_order(indptr: np.ndarray, indices: np.ndarray, data: np.ndarray) -> bool:
    n_edges = int(indices.shape[0])
    for start in range(0, n_edges, _EDGE_VALIDATION_CHUNK_SIZE):
        stop = min(start + _EDGE_VALIDATION_CHUNK_SIZE, n_edges)
        edge_offsets = np.arange(start, stop, dtype=np.int64)
        source_indices = np.searchsorted(indptr, edge_offsets, side="right") - 1
        destinations = indices[start:stop]
        if np.any(destinations < source_indices):
            return True
        if np.any((destinations == source_indices) & (data[start:stop] != 0)):
            return True
    return False
