...
def _product_args(operation: str, dtype=np.float32, n_cols: int = 2) -> tuple:
    indptr = jnp.asarray(np.array([0, 1, 1], dtype=np.int32))
    indices = jnp.asarray(np.array([1], dtype=np.int32))
    data = jnp.asarray(np.ones(1, dtype=np.int32))
    nonunique_indices = jnp.asarray(np.array([0, 1], dtype=np.int32))
    variant_indices = jnp.asarray(np.array([0, 0, 1], dtype=np.int32))
    sample_indices = jnp.asarray(np.array([1, 0], dtype=np.int32))
    flip = jnp.asarray(np.array([False, True, False], dtype=np.bool_))
    input_rows = 3 if operation == "matmat" else 2
    values = jnp.asarray(np.ones((input_rows, n_cols), dtype=dtype))
    return (
        indptr,
        indices,
        data,
...
@pytest.mark.parametrize(
    ("operation", "expected_target", "expected_rows"),
    [
        ("matmat", "linear_dag_jaxlinarg_matmat_f32", 2),
        ("rmatmat", "linear_dag_jaxlinarg_rmatmat_f32", 3),
    ],
)
def test_ffi_product_wrapper_returns_result_and_hides_workspace(
    monkeypatch,
    operation,
    expected_target,
    expected_rows,
    ...
    def fake_ffi_call(target_name, result_shape_dtypes, **kwargs):
        ...
        def call(*args, **attrs):
            captured["args"] = args
            captured["attrs"] = attrs
            result = jnp.full(result_shape_dtypes[0].shape, 7, dtype=result_shape_dtypes[0].dtype)
            workspace = jnp.zeros(result_shape_dtypes[1].shape, dtype=result_shape_dtypes[1].dtype)
    ...
    result = getattr(ffi_cpu, f"ffi_cpu_{operation}")(*_product_args(operation))

    assert captured["target_name"] == expected_target
    assert captured["result_shape_dtypes"] == (
        jax.ShapeDtypeStruct((expected_rows, 2), jnp.float32),
        jax.ShapeDtypeStruct((3, 2), jnp.float32),
    )
    assert captured["kwargs"] == {"vmap_method": "sequential"}
    assert captured["attrs"] == {"min_index_to_keep": 0}
    np.testing.assert_array_equal(np.asarray(result), np.full((expected_rows, 2), 7, dtype=np.float32))
