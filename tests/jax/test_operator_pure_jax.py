...
def test_graph_structure_validation_does_not_expand_sources_per_edge(monkeypatch):
    def fail_repeat(*args, **kwargs):
        raise AssertionError("validation must not materialize one source index per edge")

    monkeypatch.setattr(jaxlinarg_operator.np, "repeat", fail_repeat)

    JaxLinearARG.from_lineararg_arrays(**_minimal_operator_kwargs())


def test_validated_array_ingress_uses_explicit_device_placement(monkeypatch):
    placed = []
    real_device_put = jaxlinarg_operator.jax.device_put

    def tracking_device_put(value):
        placed.append(value)
        return real_device_put(value)

    monkeypatch.setattr(jaxlinarg_operator.jax, "device_put", tracking_device_put)
