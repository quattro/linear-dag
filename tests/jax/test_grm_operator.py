...
def _minimal_jax_operator(*, allele_counts=None, n_samples: int = 1) -> JaxLinearARG:
    if n_samples == 0:
        return JaxLinearARG.from_lineararg_arrays(
            ...
            flip=np.array([False]),
            sample_indices=np.array([], dtype=np.int32),
            nonunique_indices=np.array([0], dtype=np.int32),
            allele_counts=np.array([0], dtype=np.int32),
            n_variants=1,
            n_samples=0,
            backend=Backend.PURE_JAX,
        )
...
def test_jax_grm_rejects_zero_samples():
    with pytest.raises(ValueError, match="positive sample count"):
        JaxGRMOperator(_minimal_jax_operator(n_samples=0))


@pytest.mark.parametrize("allele_counts", [None, [-2], [2]])
def test_jax_grm_rejects_missing_or_invalid_allele_counts(allele_counts):
    with pytest.raises(ValueError, match="allele_counts"):
        JaxGRMOperator(_minimal_jax_operator(allele_counts=allele_counts))


@pytest.mark.parametrize("allele_count", [0, 1])
def test_jax_grm_accepts_monomorphic_allele_counts(allele_count):
    grm = JaxGRMOperator(_minimal_jax_operator(allele_counts=[allele_count]))

    np.testing.assert_array_equal(np.asarray(grm.matmat(jnp.ones((1, 1), dtype=jnp.float32))), np.zeros((1, 1)))
