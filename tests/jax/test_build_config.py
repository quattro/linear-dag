...
import linear_dag
import linear_dag.core.jaxlinarg as jaxlinarg
...
def test_jax_diagnostics_and_grm_stay_in_jax_namespace():
    assert linear_dag.Backend is jaxlinarg.Backend
    assert linear_dag.JaxLinearARG is jaxlinarg.JaxLinearARG
    assert linear_dag.JaxParallelOperator is jaxlinarg.JaxParallelOperator
    assert jaxlinarg.show_build_config is build_config.show_build_config
    assert not hasattr(linear_dag, "show_build_config")
    assert not hasattr(linear_dag, "JaxGRMOperator")
