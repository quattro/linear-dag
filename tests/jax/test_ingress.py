...
def test_ffi_cpu_ingress_keeps_exact_edge_weights(monkeypatch, oracle_case) -> None:
    monkeypatch.setattr(
        "linear_dag.core.jaxlinarg.operator.ffi_cpu.is_ffi_cpu_available",
        lambda: True,
    )
    op = from_lineararg(oracle_case.linarg, backend=Backend.FFI_CPU)

    assert op.data.dtype == jnp.int32


def test_ffi_cpu_ingress_rejects_fractional_edge_weights(monkeypatch, oracle_case) -> None:
    monkeypatch.setattr(
        "linear_dag.core.jaxlinarg.operator.ffi_cpu.is_ffi_cpu_available",
        lambda: True,
    )
    linarg = oracle_case.linarg
    data = linarg.A.data.astype(np.float32)
    data[0] = 0.5

    with np.testing.assert_raises_regex(ValueError, "integer LinearARG edge weights"):
...
def test_read_hdf5_block_arrays_honors_requested_data_dtype(linarg_h5_path, first_block_name) -> None:
    arrays = read_hdf5_block_arrays(linarg_h5_path, first_block_name, dtype=jnp.float64)

    assert arrays.data.dtype == np.float64


def test_ffi_hdf5_ingress_keeps_raw_integer_edge_weights(monkeypatch, linarg_h5_path, first_block_name) -> None:
    monkeypatch.setattr(
        "linear_dag.core.jaxlinarg.operator.ffi_cpu.is_ffi_cpu_available",
        lambda: True,
    )

    op = JaxLinearARG.from_hdf5_block(linarg_h5_path, first_block_name, backend=Backend.FFI_CPU)
