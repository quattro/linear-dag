...
This approved implementation increment ends with a private packed candidate and an evidence-based promotion decision. It does not rename the candidate, change public exports, or reroute the CLI. If every promotion gate passes, those public API changes require a separate approved implementation plan based on the collected evidence.
...
2. The packed candidate's compiled contract passes the operator explicitly as a dynamic argument and has the shape of the eventual `JaxLinearARG` API. `jit` and transformed `jit` compositions contain no graph-sized closed-over constants. Bound methods remain available as conveniences, with an explicit safe compilation helper for callers who do not want a functional call surface. This increment does not change public class identity or exports.
...
6. The currently available pure-JAX and CPU-FFI kernels share one project-owned operation contract. A future accelerator adapter must satisfy that same contract before it is exposed. HiJAX is isolated behind a private compatibility boundary and does not appear in the public API. The design can replace or remove HiJAX without changing user code.
7. The HDF5 schema remains unchanged. Ingress streams source blocks into their final packed shards and reports load balance, padding, and resident-byte metrics. Real Zarr integration remains a downstream `genoio` merge gate rather than importing that branch's storage implementation into `jax-focused`. The current exact-ragged `JaxParallelOperator` remains available until numerical, transform, memory, IR, and performance gates pass.
8. The promotion suite can validate the prototype on arm64 and x86_64 CPU hosts and on an available GPU. Benchmarks report cold compilation separately from warm execution and compare the packed implementation with both the retained exact-ragged implementation at the candidate commit and NumPy/Cython baselines. Missing or failed platform evidence forces a documented coexistence decision rather than leaving the implementation increment incomplete.
9. The package supports Python `>=3.12,<3.15` and pins the prototype to JAX/JAXlib 0.11.0 until compatibility tests justify a wider range. Direct NumPy and SciPy minimums satisfy JAX 0.11.0 requirements.

Out of scope:
- Differentiating graph topology, edge values, allele metadata, or packing decisions.
- Changing the durable LinearARG HDF5 schema, or importing the `genoio` branch's experimental Zarr implementation into this branch.
...
- Make an explicit packed LinearARG candidate a valid dynamic operand to compiled JAX programs and shape it for a later `JaxLinearARG` promotion.
...
- Reuse the current pure-JAX and CPU-FFI backends, HDF5 reader, block metadata, balancing logic, and algebraic oracle.
...
- `src/linear_dag/core/jaxlinarg/wrapper.py` on `jax-focused` assigns contiguous block ranges by `n_entries`, creates HDF5 and generic group-reader blocks directly on their assigned device, validates exclusive residency, and launches one cached exact-shape program per device range. These placement checks, ingress discipline, and balance metadata remain the memory-correctness oracle; the generic group seam is not durable Zarr support.
...
- `src/linear_dag/core/jaxlinarg/ingress.py` is the existing host-array/HDF5 side-effect boundary; its duck-typed group helper is not evidence of real Zarr support. It remains the Imperative Shell. Packing calculations are a Functional Core; mesh placement and collective orchestration remain a justified Mixed boundary.
...
The operator is always a dynamic operand of compiled code. A private primitive expands to `shard_map`, where local graph fields and descriptors are explicit mapped operands. The local body invokes the selected pure-JAX or CPU-FFI kernel. A future accelerator kernel must plug into the same operation contract.
...
| --- | --- | --- | --- | --- |
| PORT-BHV-1 | numerics | Cython and JAX compute $XW$ and $X^TY$ with flip correction. | Same logical results across packed pure-JAX and CPU-FFI paths; future accelerators inherit the same gate. | Existing oracle/fixture suite plus packed-vs-exact-ragged differential tests. |
...
| PORT-BHV-4 | io | HDF5 ingress constructs exact block objects directly on assigned devices. The branch has no real Zarr reader or schema. | Streaming HDF5/in-memory ingress fills final packed local shards without a full default-device copy. | Peak-residency and final-sharding tests for HDF5; downstream Zarr integration is gated separately. |
...
- Chosen strategy: packed globally sharded graph arrays as the foundational representation; explicit functional operator calls; `shard_map` for per-device execution; a private HiJAX primitive/type layer for transformation rules; current pure-JAX and CPU-FFI kernels for local solves.
...
- Blocking constraints: equal local shapes introduce padding; logical variant reconstruction introduces dense-result communication; HiJAX version stability is uncertain; the CPU-FFI path must accept the chosen local packed layout or use bounded local views; future accelerator support requires a separate backend implementation and hardware validation.
...
## Layer Contracts
### Ingress
- Contract: HDF5 or in-memory LinearARG blocks plus a concrete single-host mesh produce validated host packing metadata and a fixed set of globally sharded device arrays. The HDF5 schema remains unchanged. Loading is streamed by assigned shard; full graph state is never first materialized on the default device. The `genoio` Zarr reader must later satisfy this ingress contract without being merged into the JAX-focused implementation phase.
...
| --- | --- | --- | --- |
| HDF5 block datasets | assigned host shard staging buffers | single-copy fallback | The branch's durable format is block-oriented; each source block is read once into its assigned aggregate shard rather than into a default-device operator. |
...
**Covered ACs:** `jax-packed-sharded-lineararg.AC5.*`, `jax-packed-sharded-lineararg.AC7.4`, `jax-packed-sharded-lineararg.AC7.6`.
...
<!-- START_PHASE_5 -->
### Phase 5: CPU-FFI local backend
**Goal:** Reuse the current CPU-FFI backend without weakening the packed sharding or derivative contracts, while leaving a stable adapter contract for future accelerators.
...
**Components:**
- The existing CPU-FFI kernel adapts to local packed views behind the same project-owned operation contract. No Pallas or other accelerator backend is reintroduced in this phase.
...
- Backend-parametrized tests reuse the same algebraic, transformation, sharding, and residency assertions as the pure-JAX path. Future accelerator backends must join this matrix before becoming available.

**Dependencies:** Phases 3 and 4; existing kernel availability.

**Done when:** pure JAX and CPU FFI pass their assigned numerical, transform, memory, and failure-path ACs without graph replication, and unavailable accelerator requests are not advertised as supported.
<!-- END_PHASE_5 -->
...
**Components:**
- HDF5 and in-memory constructors stream into final assigned shards and expose packing diagnostics; the prototype packed carrier remains private until promotion. A downstream `genoio` integration test owns real Zarr parity.
- The phase prepares the naming, signatures, compatibility tests, and documentation required for eventual promotion, but does not switch public exports or CLI defaults before Phase 7's gates.
...
**Done when:** HDF5 and in-memory ingress satisfy peak/final residency and reconstruction parity tests, exact-ragged fallback behavior is covered, the downstream Zarr gate is documented, and the public documentation matches the still-experimental contract.
<!-- END_PHASE_6 -->
...
**Components:**
- Opt-in benchmarks compare cold compilation, warm products, GRM/RHE timing, dense communication, padding, peak/final graph residency, and IR size against the retained exact-ragged implementation at the same candidate commit and NumPy/Cython. Historical `jax-focused` and `genoio` commits remain source-selection and IR-counterexample provenance, not matched performance denominators.
...
**Done when:** reproducible evidence has been evaluated for every required reference environment and the migration decision, compatibility range, retained fallback, and any failed or missing gate are documented. This implementation increment does not perform public promotion: failed or missing gates produce coexistence under AC8.5, while a passing result authorizes a separate maintainer-approved public migration plan.
<!-- END_PHASE_7 -->
...
**Compatibility:** implementation supports Python `>=3.12,<3.15` and pins JAX/JAXlib 0.11.0, the first release with the documented HiJAX derivative helpers. NumPy and SciPy minimums are raised to at least 2.1 and 1.15. Because HiJAX is experimental and JAX uses effort-based versioning, the supported JAX range remains exact until the compatibility suite passes on another release.

**Export and serialization:** durable reconstruction in this branch continues through HDF5. A later `genoio` integration must establish equivalent Zarr reconstruction and residency behavior. Compiled/exported artifacts are not promised to embed graph buffers or survive graph-file removal. Opaque process-local handle registries are intentionally excluded.
...
| 2026-08-13 | Approved for Implementation | Approved for Implementation | Implementation investigation confirmed that this branch has no current accelerator backend or durable Zarr implementation; CPU FFI and HDF5 remain in scope, `genoio` Zarr is a downstream gate, and public promotion follows Phase 7 evidence. | Codex |
| 2026-08-13 | Approved for Implementation | Approved for Implementation | Implementation-plan review scoped this increment through the promotion decision only; public renaming, exports, and CLI routing require a separate approved plan after all evidence gates pass. | Codex |
